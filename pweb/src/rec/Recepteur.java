...
public class Recepteur extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			final Document document = builder.parse(request.getServletContext().getAttribute("rapport"));
			final Element reception = document.getDocumentElement();
			a = false;
		}
		catch (IOException e)
		{
			JSONObject reception = new JSONObject(request.getServletContext().getAttribute("rapport")); 
		}
		if(a)
		{
			/*stokage xml*/
